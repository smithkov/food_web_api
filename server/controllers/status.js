...
module.exports = {
  create(req, res) {
    ...
    if (!error) {
      return query
        .add({ name: name})
        .then((status) => res.status(OK).send(status))
  ...
  delete(req, res) {
    ...
    return query
      .delete(id)
      .then((status) => res.status(OK).send({ error: false, data: id }))
      .catch((error) => res.status(SERVER_ERROR).send(error));
  },
  ...
  findPk(req, res) {
    ...
    return query
      .findPK(id)
      .then((status) => res.status(OK).send({ error: false, data: status }))
      .catch((error) => res.status(SERVER_ERROR).send(error));
  },
