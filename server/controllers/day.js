...
module.exports = {
  create(req, res) {
    ...
    if (!error) {
      return query
        .add({ name: name})
        .then((day) => res.status(OK).send(day))
  ...
  delete(req, res) {
    ...
    return query
      .delete(id)
      .then((day) => res.status(OK).send({ error: false, data: id }))
      .catch((error) => res.status(SERVER_ERROR).send(error));
  },
  ...
  findPk(req, res) {
    ...
    return query
      .findPK(id)
      .then((day) => res.status(OK).send({ error: false, data: day }))
      .catch((error) => res.status(SERVER_ERROR).send(error));
  },
